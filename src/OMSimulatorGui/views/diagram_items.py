...
class SystemBoundaryItem(QGraphicsRectItem):
  ...
  Doesn't show a "P" parameter-file badge itself: a system's own attached
  parameter files (as opposed to a child component/subsystem's, shown via a
  badge on that child's own ElementIconItem -- see _addParameterFileBadge)
  have no canvas representation at all while viewing this system's own
  contents -- only its tree entry (the "Parameter Files" row under this
  system's own node). If this same system is later viewed *from one level
  up*, as a box on its parent's canvas, that box's own "P" badge covers it
  then. Attaching one is tree-only too (right-click the system's row) -- the
  canvas has no "current system" entry point for this, only a per-element
  one via right-clicking a component/subsystem box directly.'''
