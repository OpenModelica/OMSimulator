...
namespace oms
{
  ...
  private:
    ...
    oms_status_enu_t importStartValuesFromSSV(const pugi::xml_node& node);
    oms_status_enu_t importStartValuesFromSSVHelper(std::string fileName, std::multimap<ComRef, ComRef> &mappedEntry, const pugi::xml_node& oms_snapshot);
    oms_status_enu_t importParameterMappingFromSSM(std::string fileName, std::multimap<ComRef, ComRef> &mappedEntry, const pugi::xml_node& oms_snapshot);
