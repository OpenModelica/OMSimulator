...
oms_status_enu_t oms::Model::loadSnapshot(Snapshot& snapshot)
{
  ...
  copyResources(false);
  oms_status_enu_t status = this->importFromSnapshot(snapshot);
...
{
  pugi::xml_node ssdRoot;
  if (oms_status_ok != snapshot.getResources("SystemStructure.ssd", ssdRoot))
    return oms_status_error;
