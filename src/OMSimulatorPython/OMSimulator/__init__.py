...
import ctypes, sys, os, platform, sysconfig
  ...
  def __init__(self, omslib=None, temp_directory=None):
    if omslib is None:
      ...
      if sysconfig.get_platform() == "linux-x86_64":
        OMSIMULATORLIB_STRING = "libOMSimulator.so"
      elif sysconfig.get_platform() == "mingw":
        OMSIMULATORLIB_STRING = "libOMSimulator.dll"
      else:
        OMSIMULATORLIB_STRING = "OMSimulator.dll"
      ...
      if platform.python_version() >= "3.8":
        self.obj=CDLL(os.path.join(dirname, "", OMSIMULATORLIB_STRING), winmode=0)
      else:
        self.obj=CDLL(os.path.join(dirname, "", OMSIMULATORLIB_STRING))
    else:
      if platform.python_version() >= "3.8":
