  ...
  fmi2Boolean loggingOn)
{
    ...
    initialize(comp, functions->getTLMEffort, functions->setTLMFlow);
#endif
...
fmi2Status fmi_set_real_input_derivatives(fmi2Component c, const fmi2ValueReference vr[], size_t nvr, const fmi2Integer order[], const fmi2Real value[])
{
	...
	for (k = 0; k < nvr; k++) {
        setRealDerivative(vr[k],value[k]);
	}
...
#ifdef OPENCPS_CALLBACKS
/*
fmi2Status fmi_set_interpolated_real_callback(fmi2Component c, int vr, fmi2GetTLMEffort_ft cb)
{
  setInterpolatedRealCallback(vr, cb);

  return fmi2OK;
}

fmi2Status fmi_set_time_stamped_real_callback(fmi2Component c, int vr, fmi2SetTLMFlow_ft cb)
{
  setTimeStampedRealCallback(vr, cb);
