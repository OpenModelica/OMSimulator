...
static size_t n_states = 6;     //Number of states
...
double getReal(int idx) {
    switch(idx) {
    case 1:
        return Ith(y,2)-L/2.0*sin(Ith(y,6));
    case 2:
        return Ith(y,1)-L/2.0*cos(Ith(y,6))*Ith(y,5);
    case 3:
        return F_x;
    case 4:
        return 1.5*L+Ith(y,4)-L/2.0*cos(Ith(y,6));
    case 5:
        return Ith(y,3)+L/2.0*sin(Ith(y,6))*Ith(y,5);
    case 6:
        return F_y;
    case 7:
        return Ith(y,6);
...
double getForceX(double time) {
    v_x = Ith(y,1)-L/2.0*cos(Ith(y,6))*Ith(y,5);
...
double getForceY(double time) {
    v_y = Ith(y,3)+L/2.0*sin(Ith(y,6))*Ith(y,5);
...
double getTorque(double time) {
    ...
    case 1:
    {
        ...
        double T_int = C_int - Ith(y,5)*Zc_phi;
        return -T_int;
    }
    case 2:
        double C_int,T_int;
...
static int rhs(realtype t, N_Vector yy, N_Vector yp, void *user_data)
{
    ...
    Ith(yp,3) = Fy/M;
    Ith(yp,4) = Ith(yy,3);
    Ith(yp,5) = L/2.0*cos(Ith(yy,6))*Fx/J + L/2.0*sin(Ith(yy,6))*Fy/J - B/J*Ith(yy,5) + T/J;//L/2.0*M*9.81/J*cos(Ith(y,6)) - B/J*Ith(y,5);
    Ith(yp,6) = Ith(yy,5);
...
static int rhs_ida(realtype t, N_Vector yy, N_Vector yp, N_Vector rr, void *user_data)
{
  ...
  //Update residuals
  Ith(rr,1) = Ith(yp,1) - Fx/M + 9.81;
  Ith(rr,2) = Ith(yp,2) - Ith(yy,1);
  Ith(rr,3) = Ith(yp,3) - Fy/M;
  Ith(rr,4) = Ith(yp,4) - Ith(yy,3);
  Ith(rr,5) = Ith(yp,5) - L/2.0*cos(Ith(y,6))*Fx/J - L*sin(Ith(y,6))*Fy/J + B/J*Ith(y,5) - T/J;//- L/2.0*M*9.81/J*cos(Ith(yy,6)) + B/J*Ith(y,5);;
  Ith(rr,6) = Ith(yp,6) - Ith(yy,5);
