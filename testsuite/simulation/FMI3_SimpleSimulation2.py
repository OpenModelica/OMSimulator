## status: correct
## teardown_command: rm -rf FMI3_SimpleSimulation2.ssp FMI3_SimpleSimulation2_res.mat
## linux: no
## ucrt64: yes
## win: yes
## mac: no
...
#model.list()
model.setValue(CRef('default', 'BouncingBall', 'g'), 10.81)
model.setValue(CRef('default', 'BouncingBall', 'e'), 0.9)
model.export('FMI3_SimpleSimulation2.ssp')

model2 = SSP('FMI3_SimpleSimulation2.ssp')

model2.list()
print("", flush=True)
instantiated_model = model2.instantiate() ## internally generate the json file and also set the model state like virgin,
instantiated_model.setResultFile("FMI3_SimpleSimulation2_res.mat")
