## status: correct
## teardown_command: rm -rf FMI3_SimpleSimulation3.ssp FMI3_SimpleSimulation3_res.mat
## linux: no
## ucrt64: yes
## win: yes
## mac: no

from OMSimulator import SSP, Settings, CRef, Connector, Causality, SignalType
...
model.activeVariant.system.addConnector(Connector('param1', Causality.parameter, SignalType.Float64))

#model.list()
model.setValue(CRef('default', 'param1'), 10.81)
model.addConnection(CRef('default', 'param1'), CRef('default', 'BouncingBall', 'g'))
model.export('FMI3_SimpleSimulation3.ssp')

model2 = SSP('FMI3_SimpleSimulation3.ssp')

model2.list()
print("", flush=True)
instantiated_model = model2.instantiate() ## internally generate the json file and also set the model state like virgin,
instantiated_model.setResultFile("FMI3_SimpleSimulation3_res.mat")
...
## info:    Result file: FMI3_SimpleSimulation3_res.mat (bufferSize=1)
